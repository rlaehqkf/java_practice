...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 3. 결과값 반환(응답화면 지정)
		 * 
		 * View에서 Get방식으로 요청할 시 doGet()가 호출됨!!!
		 */
		 ...
		 * 요청 처리 스텝
		 * 
		 * 1. 요청을 처리하기 위해서 요청시 전달된 값(사용자가 입력한 값)등을 뽑는다.
		 * => input요소의 name속성값=input요소의value속성값
		 * => request의 Parameter라는 속성에 담겨있음 => 여기서 뽑아내야함
		 * 
		 * 2. 뽑아낸 값을 가공해서 요청 처리를 진행해야함(Service -> DAO -> DB) -> 오늘은 생략 -> 목요일쯤
		 * 
		 * 3. 처리 결과에 따라서 성공 / 실패 응답
		 */
		/*
		 * request의 parameter에서 전달된 데이터 값을 뽑아내는 방법
		 * 
		 * - request.getParameter("name속성값") : String
		 * => 반환타입이 String / 다른 자료형으로 사용해야 할 경우 parsing을 해주어야함
		 * 
		...
		// "셀 수 없음"
		// "이승현" / ""(텍스트상자가 비어있을 경우 빈 문자열) / null(넘어온 키 값이 없다)
		// name.length()
		...
		*/
		...
		// 2단계. 가공
		//Person person = new Person(name, gender, age, city, height, food); 
		//만들었다 침
		
		// 3단계. 요청 처리(Service단 호출)
		// INSERT라고 가정
		// Controller에서(여기) Service객체를 생성한 후 Service단의 메소드를 호출 하면서 Person객체의 주소값을 전달 한 뒤
		// Service에서 DAO단을 호출하여 DB SQL문(INSERT)를 수행한 뒤 -> 정수 형태의 결과값을 반환
		...
		pw.println("<html>");
			...
			pw.println("<body>");
				...
				pw.print("성별은 ");
				if(gender == null || "선택 안 함".equals(gender)) {
					pw.println("선택을 안했습니다.<br>");
				} else if(gender.equals("남자")) {
					pw.printf("<span id='gender'>남자</span>입니다.<br>");
				} else if(gender.equals("여자")) {
					pw.printf("<span id='gender'>여자</span>입니다.<br>");
				} 
